...
import serial
import time
arduino = serial.Serial(port='COM3', baudrate=9600, timeout=.1)  # COM3 based on the port that the arduino is connected to
def write_read(x):
    arduino.write(bytes(x, 'utf-8'))  # sending flag value to arduino
    time.sleep(0.05)
    # data = arduino.readline()  # required if we are returning a value from arduino from python
    # return data 
...
def write_on_image(frame):
    ...
    if frames_elapsed < CALIBRATION_TIME:
        ...
        if hand.isWaving:
            text = "Waving"
            # this section of code comes under the line of code where hand waving detected.
            flag = 1 # If hand waving detected
            write_read(flag) # send that to arduino
            time.sleep(2) # wait two seconds before detecting hand motion again
